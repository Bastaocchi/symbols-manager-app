...
def main():
    ...
    with tab2:
        ...
        with st.form("add_symbol_form"):
            ...
            with col2:
                sector_options = [f"{code} - {info['name']}" if code else "Não selecionado" for code, info in SECTOR_MAPPING.items()]
                selected_sector = st.selectbox("Setor SPDR", sector_options, help="Selecione um setor SPDR")
                sector_spdr_value = "" if selected_sector == "Não selecionado" else selected_sector
